		...
		match current_weapon.pattern:
			"basic":
				...
				for i in range(current_weapon.number_of_entities):
					# TODO instantiate entity + call set parameters + add the entity to the scene tree
					var entity_instance: EntityBase = current_weapon.entity_scene.instantiate() as EntityBase
		...
		match current_weapon.secondary_pattern:
			"basic":
				...
				for i in range(current_weapon.secondary_number_of_entities):
					# TODO instantiate entity + call set parameters + add the entity to the scene tree
					var entity_instance: EntityBase = current_weapon.secondary_entity_scene.instantiate() as EntityBase
					set_entity_attributes(entity_type, entity_instance, spawn_position, aim_direction)
					ObjectManager.get_entity_container().add_child(entity_instance)
					pass
...
func set_entity_attributes(entity_type: String, entity_instance: EntityBase, spawn_position: Vector2, aim_direction: Vector2) -> void:
	...
	if entity_type == "primary":
		...
		var accuracy_variation_in_radians: float = deg_to_rad(current_weapon.accuracy_variation)
		var random_accuracy_variation: float = randf_range(-accuracy_variation_in_radians / 2, accuracy_variation_in_radians / 2)
		var move_direction: Vector2 = aim_direction.rotated(random_accuracy_variation)
		 # assign move direction
		entity_instance.move_direction = move_direction
		
		# calculate range
		var range: float = current_weapon.range + randf_range(-current_weapon.range_variation, current_weapon.range_variation)
		...
		# calculate damage
		var damage: float = randf_range(current_weapon.min_damage, current_weapon.max_damage)
		# assign damage
		entity_instance.damage = damage # TODO add mults
		
		# calculate crit
		var crit_chance: float = current_weapon.crit_chance # TODO add mults
		var random_crit_chance: float = randf_range(0, 1)
		# assign crit
		if random_crit_chance <= crit_chance:
			entity_instance.is_crit = true
			var crit_mult: float = current_weapon.crit_mult # TODO add mults
		...
		# calculate spawn
		var spawn_chance: float = current_weapon.spawn_chance # TODO add mults
		var random_spawn_chance: float = randf_range(0, 1)
		# assign spawn
		if random_spawn_chance <= spawn_chance:
			entity_instance.can_spawn = true
			entity_instance.spawn_scene = current_weapon.spawn_scene
			var spawn_size_mult: float = current_weapon.spawn_size_mult # TODO add mults
	...
	if entity_type == "secondary":
		# base
		# calculate move direction
		var accuracy_variation_in_radians: float = deg_to_rad(current_weapon.secondary_accuracy_variation)
		var random_accuracy_variation: float = randf_range(-accuracy_variation_in_radians / 2, accuracy_variation_in_radians / 2)
		var move_direction: Vector2 = aim_direction.rotated(random_accuracy_variation)
		 # assign move direction
		entity_instance.move_direction = move_direction
		...
		# calculate range
		var range: float = current_weapon.secondary_range + randf_range(-current_weapon.secondary_range_variation, current_weapon.secondary_range_variation)
		range *= 32 # multiply range by tile unit size (32)
		# assign range
		entity_instance.range = range # TODO add mults
		...
		# calculate damage
		var damage: float = randf_range(current_weapon.secondary_min_damage, current_weapon.secondary_max_damage)
		# assign damage
		entity_instance.damage = damage # TODO add mults
		
		# calculate crit
		var crit_chance: float = current_weapon.secondary_crit_chance # TODO add mults
		var random_crit_chance: float = randf_range(0, 1)
		# assign crit
		if random_crit_chance <= crit_chance:
			entity_instance.is_crit = true
			var crit_mult: float = current_weapon.secondary_crit_mult # TODO add mults
			entity_instance.damage *= crit_mult
		...
		# calculate spawn
		var spawn_chance: float = current_weapon.secondary_spawn_chance # TODO add mults
		var random_spawn_chance: float = randf_range(0, 1)
		# assign spawn
		if random_spawn_chance <= spawn_chance:
			entity_instance.can_spawn = true
			entity_instance.spawn_scene = current_weapon.secondary_spawn_scene
			var spawn_size_mult: float = current_weapon.secondary_spawn_size_mult # TODO add mults
			entity_instance.spawn_size_mult *= spawn_size_mult
